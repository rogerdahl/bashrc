...
red_build() {
	...
	cd "$red_root" || {
		perror 'The path to the stderred source is invalid'
		return 1
	}
	...
	red_is_compiled || {
		touch "$red_disabled"
		perror "Unable to build 'stderred' (stderr in red). "
		perror "stderr output will not be colored."
		perror "To retry, type 'red_build'"
	}
}
